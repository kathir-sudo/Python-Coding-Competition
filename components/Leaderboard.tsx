...
const Leaderboard: React.FC<LeaderboardProps> = ({ team: currentTeam }) => {
    ...
    const fetchAndSortTeams = async () => {
      ...
      const sortedTeams = allTeams
        ...
        .sort((a, b) => {
          ...
          // For tie-breaking, earlier submission is better.
          // Teams with no submissions are ranked last in a tie.
  ...
  const exportToCSV = (teamsToExport: (Team & { rank: number })[]) => {
    ...
    for (const team of teamsToExport) {
        const values = [
            team.rank,
            `"${team.name.replace(/"/g, '""')}"`, // Handle names with quotes
            team.score,
            team.lastSubmissionTimestamp ? `"${new Date(team.lastSubmissionTimestamp).toLocaleString()}"` : 'N/A'
        ];
    ...
    link.setAttribute('download', 'finished_leaderboard.csv');
    ...
    <div className="max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-8">
            <div className="text-left">
                ...
                <p className="mt-2 text-sm text-gray-400">Showing all teams that have finished the competition.</p>
            </div>
             <button
                onClick={() => exportToCSV(teamsWithRanks)}
