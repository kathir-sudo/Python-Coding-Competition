...
const App: React.FC = () => {
  ...
  const renderView = () => {
    ...
    if (currentUser.id === 'admin') {
      ...
      switch (currentView) {
        ...
        case 'thankyou':
           return <ThankYou message="Thank you for participating!" onLogout={handleLogout} />;
        default:
          return <CodingChallenge team={teamUser} competitionState={competitionState} setCurrentUser={setCurrentUser} setCurrentView={setCurrentView} />;
      }
    }
  ...
  const teamUser = currentUser && currentUser.id !== 'admin' ? (currentUser as Team) : null;
  const isCompetitionOver = !competitionState?.isActive;
  const teamDone = teamUser && (teamUser.isDisqualified || teamUser.hasFinished);
  const showThankYouScreen = !!(teamDone || (isCompetitionOver && competitionState && teamUser));

  return (
    <div className="min-h-screen">
      {!showThankYouScreen && <Header currentUser={currentUser} setCurrentUser={setCurrentUser} setCurrentView={setCurrentView} onLogout={handleLogout} competitionState={competitionState} />}
      ...
      {competitionState?.isPaused && currentUser && currentUser.id !== 'admin' && (
        <PauseOverlay />
      )}
    </div>
  );
