...
router.put('/teams/:id/status', (req, res) => {
    const { isDisqualified, hasFinished } = req.body;
    
    if (typeof isDisqualified !== 'boolean' || typeof hasFinished !== 'boolean') {
        return res.status(400).json({ message: "isDisqualified and hasFinished must be boolean values." });
    }
    
    handleTeamUpdate(req, res, { isDisqualified, hasFinished });
});
